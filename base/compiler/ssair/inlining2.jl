...
function inline_into_block!(state::CFGInliningState, block::Int)
    if state.first_bb != block
        ...
        state.bb_rename[new_range] = (1:length(new_range)) .+ length(state.new_cfg_blocks)
...
function cfg_inline_item!(item::InliningTodo, state::CFGInliningState, from_unionsplit::Bool=false)
    ...
    if need_split_before
        bb_rename_range = (1:length(inlinee_cfg.blocks)) .+ length(state.new_cfg_blocks)
        ...
        bb_rename_range = (1:length(inlinee_cfg.blocks)) .+ (length(state.new_cfg_blocks) - 1)
...
function ir_inline_unionsplit!(compact, idx, argexprs, linetable, item, boundscheck, todo_bbs)
    ...
    for ((metharg, case), next_cond_bb) in zip(item.cases, item.bbs)
        ...
        bb = next_cond_bb - 1
        finish_current_bb!(compact)
        if isa(case, DynamicCase)
            has_generic = true
            insert_node_here!(compact, GotoNode(generic_bb), Union{}, line)
            finish_current_bb!(compact)
            continue
        end
