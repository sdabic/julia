...
is_tuple_call(ir::IRCode, def) = isa(def, Expr) && is_known_call(def, tuple, ir, ir.mod)
is_tuple_call(compact::IncrementalCompact, def) = isa(def, Expr) && is_known_call(def, tuple, compact)
